...
from ..constants import TOKEN, DOMAIN, Endpoint, GROK_CONVERSATION_ITEMS_FEATURES
from ..errors import (
    AccountLocked,
    AccountSuspended,
    BadRequest,
...
class Client:
    ...
    def set_cookies(self, cookies: dict | list, clear_cookies: bool = False) -> None:
        """
        Sets cookies from either a dictionary or a browser extension cookie list format.
        You can skip the login procedure by loading saved cookies.

        Parameters
        ----------
        cookies : Union[:class:`dict`, :class:`list`]
            Either a dictionary of cookie key-value pairs or a list of browser extension cookie objects.
        clear_cookies : :class:`bool`, default=False
            Whether to clear existing cookies before setting new ones.
        ...
        >>> # Using browser extension format
        >>> with open('browser_cookies.json', 'r', encoding='utf-8') as f:
        ...     client.set_cookies(json.load(f))
        ...
        if isinstance(cookies, list):
            # Handle browser extension cookie format
            cookie_dict = {
                cookie['name']: cookie['value']
                for cookie in cookies
                if cookie.get('domain', '').endswith(('x.com', 'twitter.com'))
            }
            self.http.cookies.update(cookie_dict)
        else:
            # Handle traditional dictionary format
            self.http.cookies.update(cookies)

    def load_cookies(self, cookies: Union[dict, list]) -> None:
        """
        Loads cookies directly from a dictionary or list.
        You can skip the login procedure by loading cookies.

        Parameters
        ----------
        cookies : Union[:class:`dict`, :class:`list`]
            Either a dictionary of cookie key-value pairs or 
            a list of browser extension cookie objects.

        Examples
        --------
        >>> # Using dictionary format
        >>> client.load_cookies(cookie_dict)
        >>> 
        >>> # Using browser extension format
        >>> client.load_cookies(cookie_list)
    ...
    async def is_logged_in(self) -> bool:
        ...
        """
        try:
            # If user information is available, it means the session is active
            user = await self.user()
            return user is not None
        except Exception:
            # If any error occurs, consider the session inactive
            return False
    ...
    async def get_notifications(
        ...
        for entry in entries:
            ...
            notification_data = entry.get('content', {}).get('item', {}).get('content', {})
            ...
            # Get tweet if exists
            tweet_data = notification_data.get('tweet', {})

            if tweet_data:
                tweet_id = tweet_data.get('id')
                if tweet_id in tweets:
                    tweet = tweets[tweet_id]
                    # Get full_text from tweet data
                    if tweet_id in global_objects['tweets']:
                        message = global_objects['tweets'][tweet_id]['full_text']
                    
            # Get user if exists        

            user_data = notification_data.get('user', {})
            if user_data:
            ...
            if 'id' not in notification_data:
                notification_data['id'] = entry['entryId']
                
            # Add timestampMs if missing
            if 'timestampMs' not in notification_data:
                notification_data['timestampMs'] = str(int(entry.get('sortIndex', 0)))

            # Add default icon if missing
            if 'icon' not in notification_data:
                notification_data['icon'] = {}

            # Set message from tweet's full_text
            notification_data['message'] = {'text': message}
                    
            # Get user data from global objects
            user_id = user_data.get('id_str')
            if user_id in global_objects.get('users', {}):
                user_data = global_objects['users'][user_id]
                user_data['rest_id'] = user_data['id_str']
                user_data['legacy'] = {
                ...
                user_data['is_blue_verified'] = user_data.get('ext_is_blue_verified', False)
                user = User(self, user_data)

            notifications.append(Notification(self, notification_data, tweet, user))
    ...
    async def grok_add_response(self, responses, conversation_id, model, image_generation_count):
        if responses and len(responses) > 0 and "message" in responses[-1]: # type: ignore
            message = responses[-1]["message"] # type: ignore
        data = {
                "responses": [
                    {
                        "message": message,
        ...
        tid = self.client_transaction.generate_transaction_id(method='POST', path=urlparse("https://grok.x.com/2/grok/add_response.json").path)
        ...
        async with self.http.stream(
                    ...
                    while True:
                        ...
                        try:
                            ...
                            if json_buffer.strip().startswith('{') and '}' in json_buffer:
                                ...
                                matches = re.search(r'({[^{}]*(?:{[^{}]*}[^{}]*)*})', json_buffer)
                                if matches:
                                    json_obj = matches.group(1)
                                    try:
                                        result = json.loads(json_obj)
                                        yield result
                                        # Remove the processed object from the buffer
                                        json_buffer = json_buffer[matches.end():]
                                        # Continue to process any remaining objects
                                        continue
                                    except json.JSONDecodeError:
                                        # If we still can't parse, wait for more data
                                        break
                            # If no complete JSON found, wait for more data
                            break
                    
                except Exception as e:
                    print(f"Error processing chunk: {e}")
                    # If there was an error, we keep going to process more chunks
                    continue
            ...
            if json_buffer.strip():
                try:
                    ...
                    # Try to find any valid JSON objects in the remaining buffer
                    for match in re.finditer(r'({[^{}]*(?:{[^{}]*}[^{}]*)*})', json_buffer):
                        try:
